...
int mg_base64_decode(const char *src, int n, char *dst) {
  ...
  while (src != NULL && src + 3 < end) {
    int a = mg_b64rev(src[0]), b = mg_b64rev(src[1]), c = mg_b64rev(src[2]), d = mg_b64rev(src[3]);
...
static void mg_sendnsreq(struct mg_connection *c, struct mg_str *name, int ms, struct mg_dns *dnsc,
  ...
  if (dnsc->c == NULL) {
    ...
    d->c = c;
    c->is_resolving = 1;
    MG_VERBOSE(("%lu resolving %.*s @ %s, txnid %hu", c->id, (int)name->len, name->ptr, dnsc->url,
                d->txnid));
...
static const char *skip(const char *s, const char *e, const char *d, struct mg_str *v) {
  v->ptr = s;
  while (s < e && *s != '\n' && strchr(d, *s) == NULL)
    s++;
...
static void mg_http_parse_headers(const char *s, const char *end, struct mg_http_header *h,
                                  int max_headers) {
  int i;
...
int mg_http_parse(const char *s, size_t len, struct mg_http_message *hm) {
  ...
  if (hm->body.len == (size_t)~0 && !is_response && mg_vcasecmp(&hm->method, "PUT") != 0 &&
...
void mg_http_serve_file(struct mg_connection *c, struct mg_http_message *hm, const char *path,
  ...
  if (fd == NULL || fs->st(path, &size, &mtime) == 0) {
    ...
    mg_printf(c,
              ...
              "Content-Length: %llu\r\n"
              "%s%s%s\r\n",
              status, mg_http_status_code_str(status), (int)mime.len, mime.ptr, etag, (uint64_t)cl,
...
static int uri_to_path2(struct mg_connection *c, struct mg_http_message *hm, struct mg_fs *fs,
  ...
  if (n + 2 < path_size)
    path[n++] = '/', path[n] = '\0';
  mg_url_decode(hm->uri.ptr + url.len, hm->uri.len - url.len, path + n, path_size - n, 0);
  ...
  MG_VERBOSE(("%lu %.*s -> %s %d", c->id, (int)hm->uri.len, hm->uri.ptr, path, flags));
  if (flags == 0) {
...
static struct mg_str stripquotes(struct mg_str s) {
  return s.len > 1 && s.ptr[0] == '"' && s.ptr[s.len - 1] == '"' ? mg_str_n(s.ptr + 1, s.len - 2)
                                                                 : s;
}
...
int mg_http_status(const struct mg_http_message *hm) { return atoi(hm->uri.ptr); }
...
static void deliver_chunked_chunks(struct mg_connection *c, size_t hlen, struct mg_http_message *hm,
                                   bool *next) {
  //  |  ... headers ... | HEXNUM\r\n ..data.. \r\n | ......
  //  +------------------+--------------------------+----
  //  |      hlen        |           chunk1         | ......
...
static void deliver_normal_chunks(struct mg_connection *c, size_t hlen, struct mg_http_message *hm,
                                  bool *next) {
...
char *mg_json_get_str(struct mg_str json, const char *path) {
  ...
  if (off >= 0 && len > 1 && json.ptr[off] == '"') {
    if ((result = (char *)calloc(1, (size_t)len)) != NULL &&
        !mg_json_unescape(mg_str_n(json.ptr + off + 1, (size_t)(len - 2)), result, (size_t)len)) {
      free(result);
      result = NULL;
    }
  }
...
static void mg_send_mqtt_properties(struct mg_connection *c, struct mg_mqtt_prop *props,
                                    size_t nprops) {
...
void mg_mqtt_pub(struct mg_connection *c, const struct mg_mqtt_opts *opts) {
  ...
  MG_DEBUG(("%lu [%.*s] -> [%.*s]", c->id, (int)opts->topic.len, (char *)opts->topic.ptr,
            (int)opts->message.len, (char *)opts->message.ptr));
  if (opts->qos > 0)
    len += 2;
  if (c->is_mqtt5)
      ...
      if (rc == MQTT_MALFORMED) {
        ...
        case MQTT_CMD_PUBLISH: {
          ...
          if (mm.qos > 0) {
            ...
            mg_mqtt_send_header(c, mm.qos == 2 ? MQTT_CMD_PUBREC : MQTT_CMD_PUBACK, 0,
                                remaining_len);
...
static bool mg_aton6(struct mg_str str, struct mg_addr *addr) {
  ...
  if (mg_v4mapped(str, addr))
    return true;
  for (i = 0; i < str.len; i++) {
    if ((str.ptr[i] >= '0' && str.ptr[i] <= '9') || (str.ptr[i] >= 'a' && str.ptr[i] <= 'f') ||
...
void mg_rpc_vok(struct mg_rpc_req *r, const char *fmt, va_list *ap) {
  ...
  if (off > 0) {
    mg_xprintf(r->pfn, r->pfn_data, "{%m:%.*s,%m:", mg_print_esc, 0, "id", len, &r->frame.ptr[off],
...
void mg_rpc_verr(struct mg_rpc_req *r, int code, const char *fmt, va_list *ap) {
  ...
  mg_xprintf(r->pfn, r->pfn_data, "{");
  if (off > 0) {
    mg_xprintf(r->pfn, r->pfn_data, "%m:%.*s,", mg_print_esc, 0, "id", len, &r->frame.ptr[off]);
  }
...
static void sntp_cb(struct mg_connection *c, int ev, void *evd, void *fnd) {
  if (ev == MG_EV_READ) {
    ...
    if (milliseconds > 0) {
      ...
      MG_VERBOSE(("%u.%u", (unsigned)(milliseconds / 1000), (unsigned)(milliseconds % 1000)));
    }
...
bool mg_send(struct mg_connection *c, const void *buf, size_t len) {
  if (c->is_udp) {
    ...
    MG_DEBUG(("%lu %p %d:%d %ld err %d", c->id, c->fd, (int)c->send.len, (int)c->recv.len, n,
              MG_SOCK_ERR(n)));
...
static void read_conn(struct mg_connection *c) {
  ...
  if (c->recv.len >= MG_MAX_RECV_SIZE) {
    ...
    MG_DEBUG(("%lu %p snd %ld/%ld rcv %ld/%ld n=%ld err=%d", c->id, c->fd, (long)c->send.len,
...
static void accept_conn(struct mg_mgr *mgr, struct mg_connection *lsn) {
  ...
  if (fd == MG_INVALID_SOCKET) {
    ...
    MG_DEBUG(("%lu %p accepted %M -> %M", c->id, c->fd, mg_print_ip_port, &c->rem, mg_print_ip_port,
              &c->loc));
    mg_call(c, MG_EV_OPEN, NULL);
...
static bool mg_socketpair(MG_SOCKET_TYPE sp[2], union usa usa[2], bool udp) {
  ...
  *(uint32_t *)&usa->sin.sin_addr = mg_htonl(0x7f000001U); // 127.0.0.1
  ...
  if (udp && (sp[0] = socket(AF_INET, SOCK_DGRAM, 0)) != MG_INVALID_SOCKET &&
      ...
      getsockname(sp[0], &usa[0].sa, &n) == 0 && getsockname(sp[1], &usa[1].sa, &n) == 0 &&
      connect(sp[0], &usa[1].sa, n) == 0 && connect(sp[1], &usa[0].sa, n) == 0) {
    success = true;
  } else if (!udp && (sock = socket(AF_INET, SOCK_STREAM, 0)) != MG_INVALID_SOCKET &&
             bind(sock, &usa[0].sa, n) == 0 && listen(sock, MG_SOCK_LISTEN_BACKLOG_SIZE) == 0 &&
...
int mg_casecmp(const char *s1, const char *s2) { return mg_ncasecmp(s1, s2, (size_t)~0); }
...
static size_t mg_nce(const char *s, size_t n, size_t ofs, size_t *koff, size_t *klen, size_t *voff,
                     size_t *vlen, char delim) {
  size_t kvlen, kl;
...
void mg_tls_init(struct mg_connection *c, const struct mg_tls_opts *opts) {
  ...
  if (opts->cert != NULL && opts->cert[0] != '\0') {
    ...
    rc = mbedtls_pk_parse_key(&tls->pk, (uint8_t *)s.ptr, s.len + 1, NULL, 0 MGRNG);
    if (key[0] != '-')
      free((char *)s.ptr);
    if (rc != 0) {
...
void mg_tls_init(struct mg_connection *c, const struct mg_tls_opts *opts) {
  ...
  MG_DEBUG(("%lu Setting TLS, CA: %s, cert: %s, key: %s", c->id,
            opts->ca == NULL ? "null" : opts->ca, opts->cert == NULL ? "null" : opts->cert,
            opts->certkey == NULL ? "null" : opts->certkey));
  tls->ctx =
      c->is_client ? SSL_CTX_new(SSLv23_client_method()) : SSL_CTX_new(SSLv23_server_method());
  ...
  if (opts->cert != NULL && opts->cert[0] != '\0') {
    ...
    } else if ((rc = SSL_use_certificate_chain_file(tls->ssl, opts->cert)) != 1) {
...
static uint32_t imxrt1020_eth_read_phy(uint8_t addr, uint8_t reg) {
  ...
  phy_transaction = (0x1 << 30) | (0x2 << 28) | ((uint32_t)(addr & mask_phy_adr_reg) << 23) |
...
static void imxrt1020_eth_write_phy(uint8_t addr, uint8_t reg, uint32_t val) {
  ...
  val &= mask_phy_data;
  uint32_t phy_transaction = 0x00;
  phy_transaction = (uint32_t)(0x1 << 30) | (uint32_t)(0x1 << 28) | (uint32_t)(addr << 23) |
                    (uint32_t)(reg << 18) | (uint32_t)(0x2 << 16) | (uint32_t)(val);
  ENET->MMFR = phy_transaction;
  wait_phy_complete();
}
...
enet_bd_struct_t rx_buffer_descriptor[(ENET_RXBD_NUM)] __attribute__((aligned((64U))));
__attribute__((section("NonCacheable,\"aw\",%nobits @")))
enet_bd_struct_t tx_buffer_descriptor[(ENET_TXBD_NUM)] __attribute__((aligned((64U))));

uint8_t rx_data_buffer[(ENET_RXBD_NUM)][((unsigned int)(((ENET_RXBUFF_SIZE)) + (((64U)) - 1U)) &
                                         (unsigned int)(~(unsigned int)(((64U)) - 1U)))]
    __attribute__((aligned((64U))));
uint8_t tx_data_buffer[(ENET_TXBD_NUM)][((unsigned int)(((ENET_TXBUFF_SIZE)) + (((64U)) - 1U)) &
                                         (unsigned int)(~(unsigned int)(((64U)) - 1U)))]
    __attribute__((aligned((64U))));
...
static bool mg_tcpip_driver_imxrt1020_init(struct mg_tcpip_if *ifp) {
  ...
  // PHY: Start Link
  {
    ...
    imxrt1020_eth_write_phy(IMXRT1020_PHY_ADDR, 0x1f,
...
static size_t mg_tcpip_driver_imxrt1020_tx(const void *buf, size_t len, struct mg_tcpip_if *ifp) {
  ...
  if (len > sizeof(tx_data_buffer[ENET_TXBD_NUM])) {
    ...
    //  tx_buffer_descriptor[s_rt1020_txno].control &= (uint16_t)(BIT(14) | BIT(12)); // Own doesn't
    //  affect HW
    ...
    // Read inframes
      ...
      for (uint32_t i = 0; i < 10; i++) { // read as they arrive but not forever
        ...
        if (!(rx_buffer_descriptor[s_rt1020_rxno].control & (BIT(2) | BIT(0)))) {
          ...
          mg_tcpip_qwrite(rx_buffer_descriptor[s_rt1020_rxno].buffer, len > 4 ? len - 4 : len,
                          s_ifp);
        }
...
// API
struct mg_tcpip_driver mg_tcpip_driver_imxrt1020 = {mg_tcpip_driver_imxrt1020_init,
                                                    mg_tcpip_driver_imxrt1020_tx, NULL,
...
static size_t mg_tcpip_driver_stm32_tx(const void *buf, size_t len, struct mg_tcpip_if *ifp) {
...
static bool mg_tcpip_driver_stm32_up(struct mg_tcpip_if *ifp) {
  ...
  if ((ifp->state == MG_TCPIP_STATE_DOWN) && up) { // link state just went up
    ...
    MG_DEBUG(
        ("Link is %uM %s-duplex", maccr & BIT(14) ? 100 : 10, maccr & BIT(11) ? "full" : "half"));
  }
  return up;
}
...
struct mg_tcpip_driver mg_tcpip_driver_stm32 = {
    mg_tcpip_driver_stm32_init, mg_tcpip_driver_stm32_tx, NULL, mg_tcpip_driver_stm32_up};
#endif
...
static uint32_t get_hclk(void) {
  struct rcc {
    volatile uint32_t CR, HSICFGR, CRRCR, CSICFGR, CFGR, RESERVED1, D1CFGR, D2CFGR, D3CFGR,
        RESERVED2, PLLCKSELR, PLLCFGR, PLL1DIVR, PLL1FRACR, PLL2DIVR, PLL2FRACR, PLL3DIVR,
        PLL3FRACR, RESERVED3, D1CCIPR, D2CCIP1R, D2CCIP2R, D3CCIPR, RESERVED4, CIER, CIFR, CICR,
        RESERVED5, BDCR, CSR, RESERVED6, AHB3RSTR, AHB1RSTR, AHB2RSTR, AHB4RSTR, APB3RSTR,
        APB1LRSTR, APB1HRSTR, APB2RSTR, APB4RSTR, GCR, RESERVED8, D3AMR, RESERVED11[9], RSR,
        AHB3ENR, AHB1ENR, AHB2ENR, AHB4ENR, APB3ENR, APB1LENR, APB1HENR, APB2ENR, APB4ENR,
        RESERVED12, AHB3LPENR, AHB1LPENR, AHB2LPENR, AHB4LPENR, APB3LPENR, APB1LLPENR, APB1HLPENR,
        APB2LPENR, APB4LPENR, RESERVED13[4];
  } *rcc = ((struct rcc *)(0x40000000 + 0x18020000 + 0x4400));
  uint32_t clk = 0, hsi = 64000000 /* 64 MHz */, hse = 8000000 /* 8MHz */, csi = 4000000 /* 4MHz */;
...
static bool mg_tcpip_driver_stm32h_up(struct mg_tcpip_if *ifp) {
  ...
  if ((ifp->state == MG_TCPIP_STATE_DOWN) && up) { // link state just went up
    ...
    ETH->MACCR = maccr;  // IRQ handler does not fiddle with this register
    MG_DEBUG(
        ("Link is %uM %s-duplex", maccr & BIT(14) ? 100 : 10, maccr & BIT(13) ? "full" : "half"));
  }
  return up;
}
...
static struct ip *tx_ip(struct mg_tcpip_if *ifp, uint8_t *mac_dst, uint8_t proto, uint32_t ip_src,
...
static void tx_udp(struct mg_tcpip_if *ifp, uint8_t *mac_dst, uint32_t ip_src, uint16_t sport,
...
static void tx_dhcp(struct mg_tcpip_if *ifp, uint8_t *mac_dst, uint32_t ip_src, uint32_t ip_dst,
...
static void tx_dhcp_request(struct mg_tcpip_if *ifp, uint8_t *mac_dst, uint32_t ip_src,
                            uint32_t ip_dst) {
  uint8_t opts[] = {
...
static struct mg_connection *getpeer(struct mg_mgr *mgr, struct pkt *pkt, bool lsn) {
  ...
  for (c = mgr->conns; c != NULL; c = c->next) {
    if (c->is_udp && pkt->udp && c->loc.port == pkt->udp->dport)
      break;
    if (!c->is_udp && pkt->tcp && c->loc.port == pkt->tcp->dport && lsn == c->is_listening &&
    ...
    *arp = *pkt->arp;
    ...
    if (pkt->arp->spa == ifp->gw) {
      ...
      if (c != NULL && c->is_arplooking) {
        ...
        MG_DEBUG(
            ("%lu ARP resolved %M -> %M", c->id, mg_print_ip4, c->rem.ip, mg_print_mac, s->mac));
        c->is_arplooking = 0;
      }
    }
  }
...
static void rx_dhcp_server(struct mg_tcpip_if *ifp, struct pkt *pkt) {
  uint8_t op = 0, *p = pkt->dhcp->options, *end = (uint8_t *)&pkt->raw.ptr[pkt->raw.len];
  if (end < (uint8_t *)(pkt->dhcp + 1))
    return;
  ...
  if (op == 1 || op == 3) {        // DHCP Discover or DHCP Request
    ...
    // memcpy(ifp->gwmac, pkt->eth->src, sizeof(ifp->gwmac));
    tx_udp(ifp, pkt->eth->src, ifp->ip, mg_htons(67), op == 1 ? ~0U : res.yiaddr, mg_htons(68),
           &res, sizeof(res));
  }
}
...
static size_t tx_tcp(struct mg_tcpip_if *ifp, uint8_t *dst_mac, uint32_t dst_ip, uint8_t flags,
                     uint16_t sport, uint16_t dport, uint32_t seq, uint32_t ack, const void *buf,
                     size_t len) {
  ...
  tcp->csum = csumfin(cs);
  MG_DEBUG(("TCP %M:%hu -> %M:%hu fl %x len %u", mg_print_ip4, &ip->src, mg_ntohs(tcp->sport),
            mg_print_ip4, &ip->dst, mg_ntohs(tcp->dport), tcp->flags, (int)len));
  return ether_output(ifp, PDIFF(ifp->tx.ptr, tcp + 1) + len);
}

static size_t tx_tcp_pkt(struct mg_tcpip_if *ifp, struct pkt *pkt, uint8_t flags, uint32_t seq,
                         const void *buf, size_t len) {
  uint32_t delta = (pkt->tcp->flags & (TH_SYN | TH_FIN)) ? 1 : 0;
  return tx_tcp(ifp, pkt->eth->src, pkt->ip->src, flags, pkt->tcp->dport, pkt->tcp->sport, seq,
...
static struct mg_connection *accept_conn(struct mg_connection *lsn, struct pkt *pkt) {
...
long mg_io_send(struct mg_connection *c, const void *buf, size_t len) {
  ...
  if (c->is_udp) {
    ...
    if (len + max_headers_len > ifp->tx.len)
      len = ifp->tx.len - max_headers_len;
    if (tx_tcp(ifp, s->mac, rem_ip, TH_PUSH | TH_ACK, c->loc.port, c->rem.port, mg_htonl(s->seq),
               mg_htonl(s->ack), buf, len) > 0) {
      s->seq += (uint32_t)len;
...
static void rx_ip(struct mg_tcpip_if *ifp, struct pkt *pkt) {
  if (pkt->ip->proto == 1) {
    ...
    if (pkt->pay.len < sizeof(*pkt->udp))
      return;
    mkpay(pkt, pkt->udp + 1);
    MG_DEBUG(("UDP %M:%hu -> %M:%hu len %u", mg_print_ip4, &pkt->ip->src, mg_ntohs(pkt->udp->sport),
              mg_print_ip4, &pkt->ip->dst, mg_ntohs(pkt->udp->dport), (int)pkt->pay.len));
    ...
    MG_DEBUG(("TCP %M:%hu -> %M:%hu len %u", mg_print_ip4, &pkt->ip->src, mg_ntohs(pkt->tcp->sport),
              mg_print_ip4, &pkt->ip->dst, mg_ntohs(pkt->tcp->dport), (int)pkt->pay.len));
    rx_tcp(ifp, pkt);
  }
}
...
static void mg_tcpip_rx(struct mg_tcpip_if *ifp, void *buf, size_t len) {
  ...
  if (ifp->enable_mac_check && memcmp(pkt.eth->dst, ifp->mac, sizeof(pkt.eth->dst)) != 0 &&
...
static void mg_tcpip_poll(struct mg_tcpip_if *ifp, uint64_t uptime_ms) {
  ...
  if (expired_1000ms && ifp->lease_expire > 0 && ifp->ip != 0 &&
      ifp->now + 30 * 60 * 1000 > ifp->lease_expire && ((ifp->now / 1000) % 10) == 0) {
  ...
  for (struct mg_connection *c = ifp->mgr->conns; c != NULL; c = c->next) {
    ...
    if (uptime_ms > s->timer) {
      if (s->ttype == MIP_TTYPE_ACK) {
        ...
        tx_tcp(ifp, s->mac, rem_ip, TH_ACK, c->loc.port, c->rem.port, mg_htonl(s->seq),
               mg_htonl(s->ack), "", 0);
      } else {
        if (s->tmiss++ > 2) {
          ...
          tx_tcp(ifp, s->mac, rem_ip, TH_ACK, c->loc.port, c->rem.port, mg_htonl(s->seq - 1),
                 mg_htonl(s->ack), "", 0);
        }
      }
      settmout(c, MIP_TTYPE_KEEPALIVE);
...
void mg_tcpip_init(struct mg_mgr *mgr, struct mg_tcpip_if *ifp) {
  ...
  if (ifp->driver->init && !ifp->driver->init(ifp)) {
    ...
    ifp->eport |= MG_EPHEMERAL_PORT_BASE;        // Random from MG_EPHEMERAL_PORT_BASE to 65535
...
static void close_conn(struct mg_connection *c) {
  ...
  if (c->is_udp == false && c->is_listening == false) { // For TCP conns,
    ...
    tx_tcp(ifp, s->mac, rem_ip, TH_FIN | TH_ACK, c->loc.port, c->rem.port, mg_htonl(s->seq),
...
void mg_mgr_poll(struct mg_mgr *mgr, int ms) {
  ...
  for (c = mgr->conns; c != NULL; c = tmp) {
    tmp = c->next;
    mg_call(c, MG_EV_POLL, &now);
    MG_VERBOSE(("%lu .. %c%c%c%c%c", c->id, c->is_tls ? 'T' : 't', c->is_connecting ? 'C' : 'c',
                c->is_tls_hs ? 'H' : 'h', c->is_resolving ? 'R' : 'r', c->is_closing ? 'C' : 'c'));
    if (c->is_tls_hs)
      mg_tls_handshake(c);
    if (can_write(c))
